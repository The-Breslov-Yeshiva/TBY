...
type CampaignCountdownVariant = "feature" | "spotlight" | "hero" | "donate";
...
export function CampaignCountdown({ variant = "hero", className }: { variant?: CampaignCountdownVariant; className?: string }) {
  ...
  const isLarge = isFeature || isSpotlight;
  ...
  return (
    <section
      className={cn(
        ...
        isLarge ? "rounded-[2rem] p-5 sm:p-8 lg:p-10 xl:p-12" : isHero ? "p-5 sm:p-6 lg:p-7 xl:p-8" : "p-4 sm:p-5 lg:p-6",
        className
      )}
      ...
      <div
        className={cn(
          ...
          isFeature
            ...
            : isSpotlight
              ? "xl:grid-cols-[minmax(0,0.94fr)_minmax(20rem,0.78fr)] xl:items-center xl:gap-9"
            : isHero
              ? "lg:items-center"
      ...
      >
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-3">
            <span className={cn("inline-flex items-center gap-2 rounded-full border border-amber-200/25 bg-amber-200/10 px-3 py-1.5 text-xs font-black uppercase tracking-[0.18em] text-amber-200", isLarge && "px-4 py-2 text-sm")}>
              <Clock3 className={cn("h-3.5 w-3.5", isLarge && "h-4 w-4")} />
              {fundraisingCampaign.eyebrow}
            </span>
            <span className={cn("rounded-full border border-white/10 bg-white/8 px-3 py-1.5 text-xs font-bold text-gray-200", isLarge && "px-4 py-2 text-sm")}>
          ...
          <Heading
            ...
            className={cn(
              ...
              isFeature ? "text-4xl sm:text-6xl lg:text-7xl xl:text-8xl" : isSpotlight ? "text-4xl sm:text-5xl xl:text-6xl" : isHero ? "text-3xl sm:text-4xl xl:text-5xl" : "text-3xl sm:text-4xl"
            )}
          >
            <span className="gradient-text">{fundraisingCampaign.tagline}</span>
          </Heading>
          <p className={cn("mt-4 max-w-3xl leading-8 text-gray-200", isFeature ? "text-xl sm:text-2xl sm:leading-9" : isSpotlight ? "text-lg sm:text-xl sm:leading-8" : isHero ? "text-lg sm:text-xl sm:leading-8" : "text-base sm:text-lg")}>
            {fundraisingCampaign.copy}
          </p>

          <div className={cn("mt-6 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3", isFeature && "mt-8 gap-3 sm:gap-4", isSpotlight && "gap-3 sm:gap-4", isHero && "gap-3 sm:gap-3.5")} aria-live="polite">
            {units.map(([label, value]) => (
              <div key={label} className={cn("rounded-2xl border border-white/10 bg-white/[0.075] p-3 text-center backdrop-blur-md sm:p-4", isLarge && "p-4 sm:p-5", isHero && "p-4")}>
                <strong className={cn("block font-black leading-none text-white tabular-nums text-3xl sm:text-4xl", isFeature && "text-4xl sm:text-5xl lg:text-6xl", isSpotlight && "text-4xl sm:text-5xl lg:text-6xl", isHero && "text-4xl sm:text-5xl")}>
                  {typeof value === "number" ? padTime(value) : "--"}
                </strong>
                <span className={cn("mt-2 block text-[0.68rem] font-black uppercase tracking-[0.16em] text-gray-400", (isSpotlight || isHero) && "text-xs")}>{label}</span>
              </div>
            ))}
          </div>

          <div className={cn("mt-6 flex flex-col gap-3", isLarge && "mt-8", !isSpotlight && "sm:flex-row sm:items-center", isSpotlight && "sm:items-start 2xl:flex-row 2xl:items-center")}>
            <a
        ...
        <div className={cn("relative min-h-[17rem]", isFeature ? "hidden min-h-[30rem] lg:block xl:min-h-[34rem]" : isSpotlight ? "hidden min-h-[26rem] xl:block xl:translate-x-7 2xl:translate-x-10" : isHero ? "hidden" : "hidden xl:block")}>
          <motion.div
            className={cn("absolute left-0 top-5 w-[63%] overflow-hidden rounded-[1.35rem] border border-white/12 bg-white/8 shadow-cinematic", isSpotlight && "-translate-x-14")}
          ...
          <div className={cn("absolute bottom-5 left-4 rounded-2xl border border-white/12 bg-gray-950/80 p-4 shadow-cinematic backdrop-blur-xl", isSpotlight && "translate-y-6 -translate-x-16")}>
