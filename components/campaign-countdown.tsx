  ...
  return (
    <section
      className={cn(
        ...
        isFeature ? "rounded-[2rem] p-5 sm:p-8 lg:p-10 xl:p-12" : isHero ? "mt-12 p-4 sm:p-5 lg:p-6" : "p-4 sm:p-5 lg:p-6",
        className
      )}
      ...
      <div
        className={cn(
          ...
          isFeature
            ...
            : isHero
              ? "lg:grid-cols-[minmax(0,1fr)_minmax(20rem,0.9fr)] lg:items-center"
        ...
        <div className="min-w-0">
          ...
          <Heading
            ...
            className={cn(
              ...
              isFeature ? "text-4xl sm:text-6xl lg:text-7xl xl:text-8xl" : isHero ? "text-3xl sm:text-4xl lg:text-5xl" : "text-3xl sm:text-4xl"
            )}
          >
            <span className="gradient-text">{fundraisingCampaign.tagline}</span>
          </Heading>
          <p className={cn("mt-4 max-w-3xl leading-8 text-gray-200", isFeature ? "text-xl sm:text-2xl sm:leading-9" : isHero ? "text-lg sm:text-xl" : "text-base sm:text-lg")}>
            {fundraisingCampaign.copy}
          </p>

          <div className={cn("mt-6 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3", isFeature && "mt-8 gap-3 sm:gap-4")} aria-live="polite">
            {units.map(([label, value]) => (
              <div key={label} className={cn("rounded-2xl border border-white/10 bg-white/[0.075] p-3 text-center backdrop-blur-md sm:p-4", isFeature && "p-4 sm:p-5")}>
                <strong className={cn("block font-black leading-none text-white tabular-nums text-3xl sm:text-4xl", isFeature && "text-4xl sm:text-5xl lg:text-6xl")}>
                  {typeof value === "number" ? padTime(value) : "--"}
                </strong>
                <span className="mt-2 block text-[0.68rem] font-black uppercase tracking-[0.16em] text-gray-400">{label}</span>
              </div>
            ))}
          </div>
          ...
          <div className={cn("mt-6 flex flex-col gap-3 sm:flex-row sm:items-center", isFeature && "mt-8")}>
            <a
              ...
              className={cn("inline-flex items-center justify-center gap-2 rounded-full bg-white px-6 py-4 text-sm font-black text-gray-950 transition hover:bg-amber-100", isFeature && "px-8 py-5 text-base")}
            >
              Donate to the Campaign
              <ArrowUpRight className="h-4 w-4" />
            </a>
            <p className={cn("text-sm font-bold leading-6 text-gray-300", isFeature && "text-base")}>
              {isComplete ? "The countdown is complete. The campaign link remains open." : `Countdown ends ${fundraisingCampaign.endsAtLabel}.`}
            </p>
          </div>
        </div>

        <div className={cn("relative min-h-[17rem]", isFeature ? "hidden min-h-[30rem] lg:block xl:min-h-[34rem]" : isHero ? "hidden lg:block" : "hidden xl:block")}>
          <motion.div
